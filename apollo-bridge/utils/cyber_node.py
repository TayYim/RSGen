...
class CyberNode(object):
    ...
    @staticmethod
    def start_apollo_control(log):
        launch_cmd = f"nohup cyber_launch start /apollo/modules/control/launch/control.launch &"
        log.info(f"launch cmd is {launch_cmd}")
        os.system(launch_cmd)
        time.sleep(1)

    def restart_apollo_control(self, log):
        self.stop_apollo_control(log)
        self.start_apollo_control(log)
